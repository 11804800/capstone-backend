...
async function UserSignup(req,res)
{
    try{
        ...
        const isUserExist=await User.findOne({$or:[{username:req.body.username},{email:req.body.email}]});
        ...
        res.status(201).json({message:"User Registration SuccessFull",result:result});
    }
...
async function UserLogin(req,res)
{
    try
    {
        ...
        const isUser=await User.findOne({$or:[{username:username},{email:username}]});
        ...
        else
        {
            ...
            if(isValid)
            {
                ...
                const token=jwt.sign({user:isUser},process.env.SECRET_KEY,{expiresIn:"30m"});
...
async function UpdateUserInfo(req,res)
{
    try
    {
        ...
        const result=await User.findOneAndUpdate({username:req.params.username},{
            $set:req.body
        });
...
module.exports={
    GetUser,
    UserSignup,UserLogin,UpdateUserInfo,DeleteUser
};